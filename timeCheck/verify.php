...
$tempDir       = __DIR__ . '/../uploads/reference_image/';
...
// Save temp image
$raw = str_replace(['data:image/jpeg;base64,', ' '], ['', '+'], $_POST['current']);
$tempImage = $tempDir . 'temp_' . time() . '_' . mt_rand(1000,9999) . '.jpg';
...
while ($user = $usersRes->fetch_assoc()) {
    $referencePath = __DIR__ . '/../' . $user['reference_image'];
...
if ($attendance === null) {
    ...
    // Compute hours worked correctly (timestamp diff, handles overnight)
    $clockInDT   = new DateTime($attendance['clock_in']);
    $clockOutDT  = new DateTime(); // now
    $seconds     = $clockOutDT->getTimestamp() - $clockInDT->getTimestamp();
    $workedHours = max(0, $seconds / 3600); // never negative
