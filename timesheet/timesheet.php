    ...
    <div class="max-w-7xl mx-auto">
        ...
        <form method="GET" class="mb-6 flex flex-wrap gap-2 items-end">
            <div class="flex flex-col">
                ...
                <input type="text" name="employee_name" class="border rounded px-2 py-1 w-full md:w-48" placeholder="Type employee..." value="<?= htmlspecialchars($_GET['employee_name'] ?? '') ?>">
            </div>
            <div class="flex flex-col">
                <label class="text-sm">Department</label>
                <select name="department" class="border rounded px-2 py-1 w-full md:w-48">
                    <option value="">All</option>
                    <?php $deptRes->data_seek(0); while($dept = $deptRes->fetch_assoc()): ?>
                        <option value="<?= $dept['department_id'] ?>" <?= $selectedDept==$dept['department_id']?'selected':'' ?>><?= htmlspecialchars($dept['name']) ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
            <div class="flex flex-col">
                <label class="text-sm">Position</label>
                <select name="position" class="border rounded px-2 py-1 w-full md:w-48">
                    <option value="">All</option>
                    <?php $posRes->data_seek(0); while($pos = $posRes->fetch_assoc()): ?>
                        <option value="<?= $pos['position_id'] ?>" <?= $selectedPos==$pos['position_id']?'selected':'' ?>><?= htmlspecialchars($pos['title']) ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
            <div class="flex flex-col">
                <label class="text-sm">Status</label>
                <select name="status" class="border rounded px-2 py-1 w-full md:w-36">
                    <option value="">All</option>
            ...
            <div class="flex flex-col">
                ...
                <input type="date" name="start_date" class="border rounded px-2 py-1 w-full md:w-40" value="<?= htmlspecialchars($startDate) ?>">
            </div>
            <div class="flex flex-col">
                <label class="text-sm">End Date</label>
                <input type="date" name="end_date" class="border rounded px-2 py-1 w-full md:w-40" value="<?= htmlspecialchars($endDate) ?>">
            </div>
            <div class="flex flex-col gap-1">
                <button type="submit" class="bg-blue-500 text-white px-3 py-1 rounded w-full md:w-auto">Filter</button>
